...
void GestionAnimaux::on_calendrierb_clicked()
{
Cal->show();
}

void GestionAnimaux::on_archiveButton_clicked()
{
    //Archive selected rows
        QItemSelectionModel *select = ui->tablevie->selectionModel();
        if (select->hasSelection()) {
            QModelIndexList selectedRows = select->selectedRows();
            for (int i = selectedRows.count() - 1; i >= 0; --i) {
                QModelIndex index = selectedRows.at(i);
                archivedItems.append(model->record(index.row()));
                model->removeRow(index.row());
            }
            model->submitAll();
            QMessageBox::information(this, "Succès", "Les données ont été archivées avec succès.");
        } else {
            QMessageBox::warning(this, "Erreur", "Aucune donnée sélectionnée pour l'archivage.");
        }
}


void GestionAnimaux::on_unarchiveButton_clicked()
{

    // Unarchive all items
       if (!archivedItems.isEmpty()) {
           for (const QSqlRecord &record : archivedItems) {
               model->insertRecord(-1, record);
           }
           model->submitAll();
           archivedItems.clear();
           QMessageBox::information(this, "Succès", "Les données ont été désarchivées avec succès.");
       } else {
           QMessageBox::warning(this, "Erreur", "Aucune donnée à désarchiver.");
       }
